...
func TestPosition3D_Equals(t *testing.T) {
	a := pos.NewUnchecked(1, 2, 0)
	b := pos.NewUnchecked(1, 2, 0)
	c := pos.NewUnchecked(2, 2, 0)
	d := pos.NewUnchecked(1, 2, 1)
...
func TestPosition3D_DistanceManhattan(t *testing.T) {
	a := pos.NewUnchecked(0, 0, 0)
	// Déplacement : 3(x) + 4(y) + 1(z) = 8
	b := pos.NewUnchecked(3, 4, 1)
...
func TestPosition3D_DistanceChebyshev(t *testing.T) {
	...
	c := pos.NewUnchecked(1, 1, 5) // max(1,1) + 5 = 6
